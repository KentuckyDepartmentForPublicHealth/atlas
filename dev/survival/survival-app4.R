...
ui <- fluidPage(
  ...
  sidebarLayout(
    ...
    mainPanel(
      # Output survival plot and debug info
      plotOutput("survivalPlot", height = "600px"),
      verbatimTextOutput("debugInfo") # Added for debugging
    )
  )
)
...
server <- function(input, output, session) {
  ...
  filteredData <- reactive({
    ...
    # Convert grouping variable to factor to preserve all levels
  ...
  output$survivalPlot <- renderPlot({
    ...
    p <- ggsurvplot(
      ...
      break.time.by = 12,             # Break x-axis by 12 months for readability
      risk.table.y.text.col = TRUE    # Color risk table by group
    )
  ...
  # Debug output to check group sizes
