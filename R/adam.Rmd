...
filter out: `Sample analysis group` == Test - Not classified (n=79) <-same-> [`Final diagnosis`d == '-'] == 79 <-same-> 
...
atlasDataClean <- atlasDataClean %>%
  mutate(
    across( 
     ...
     str_to_upper # make char ALL CAPS
    )
)
...
atlasDataClean <- atlasDataClean %>%
  mutate(
    ...
    ageGroup = factor(ageGroup, levels = age_groups, labels = age_groups), # make factor
    recurrent = ifelse(recurrent %in% c('PRIMARY', 'RECURRENT'), recurrent, NA) %>% factor() # flip n=19 to NA because inexclusive; make factor
  )
