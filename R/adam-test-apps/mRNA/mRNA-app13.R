...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      # Slider for total unique genes
      uiOutput("max_options_slider"),
      
      # Conditional Panel for GO Term selection
      conditionalPanel(
      ...
      # Conditional Panel for All Genes selection
      conditionalPanel(
      ...
      # Use conditionalPanel to show/hide group by options
      uiOutput("groupByControls"),
...
server <- function(input, output, session) {
  ...
  # Render the group by controls conditionally based on gene selection
  output$groupByControls <- renderUI({
    if(length(selected_genes()) > 0) {
      ...
      tagList()  # Empty list if no genes are selected
    }
  })
