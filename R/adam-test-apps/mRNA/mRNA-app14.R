    ...
    sidebarPanel(
      ...
      conditionalPanel(
        ...
        selectInput("group_by", "Group By", 
                    choices = sort(c("isCancerous", "grade", "ageGroup", "tumorType", 
...
server <- function(input, output, session) {
  ...
  processed_data <- eventReactive(input$run, {
    ...
    withProgress(message = "Processing data...", value = 0, {
      ...
      full_data <- gene_expression[
        gene_annotations, on = .(SYMBOL), nomatch = 0][
          atlasDataClean, on = "filename", nomatch = 0]
  ...
  output$gene_info <- renderTable({
    data <- processed_data()
    if (nrow(data) == 0) return(data.frame(SYMBOL = character(0), GENENAME = character(0)))
