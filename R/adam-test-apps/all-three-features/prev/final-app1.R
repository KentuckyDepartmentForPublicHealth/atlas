...
server <- function(input, output, session) {
    # Within the server function, replace the Survival Analysis section:
...
# Within the server function, replace the create_survival_plot function:
...
create_survival_plot <- function(data, strat_var, title = NULL, show_pvalue = TRUE, show_ci = FALSE, max_time = NULL) {
    ...
    # Create lighter versions of colors for confidence intervals
    ...
    ci_colors <- sapply(colors, lighten_color, factor = 0.7) # Adjust factor for desired lightness
    ...
    # Add confidence intervals as shaded regions if requested
    if (show_ci) {
        for (i in seq_along(levels)) {
            ...
            # Ensure no NA values disrupt the polygon
            ...
            if (length(time) > 0) {
                ...
                polygon(c(time, rev(time)), c(lower, rev(upper)),
                    col = adjustcolor(ci_colors[i], alpha.f = 0.3), # Semi-transparent fill
                    border = NA
    ...
    # Re-plot survival lines to ensure they appear over the CI regions
    lines(surv_fit, col = colors, lwd = 2)

    # Add group counts to legend
    ...
    # Updated Group Statistics Table
    output$group_stats <- renderTable(
        {
            ...
            if (input$analysis_type == "single") {
                ...
                # For grid view, compute stats for all selected variables
...
# Within the server function, replace the mRNA Expression Boxplots logic:
# mRNA Expression Boxplots Logic
validate(
...
# Reactive value to track if plot is cleared
...
observeEvent(input$reset, {
    ...
    # Set plot_cleared to TRUE to render the cleared message
    plot_cleared(TRUE)
})

# Reset plot_cleared when a new plot is generated
