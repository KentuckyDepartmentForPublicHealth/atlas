...
fit<- survfit(Surv(Overall.survival..months.,Vital.status)~1, data=jde.survival);fit
...
```{r}
FB<-read.csv("C:/Users/henry/Downloads/FB survival.csv")
fit3<- survfit(Surv(Overall.survival..months.,FB$Vital.status..1.dead..0.alive.)~1, data=FB);fit3
plot(fit3, xlab="Time", ylab="Survival", main="FB Data", col=c(1), lty=c(1),conf.int = F)
fit4<- survfit(Surv(Overall.survival..months.,FB$Vital.status..1.dead..0.alive.)~strata(FB$Age..grouped.), data=FB)
plot(fit4, xlab="Time", ylab="Survival", main="FB Data", col=1:3, lty=c(1),conf.int = F)
```
